...
class PlayingState implements GameState{
	...
	public var switchToGameOverState:Bool = false;

    public function new(stage:Stage) {
		this.stage = stage;
	}

    public function enter(_:GameState):Void {
		bullets = Bullet.initBullets(stage);
		asteroids = Asteroid.initAsteroids(stage, asteroids);
		for (bullet in bullets) stage.addChild(bullet);
		for (asteroid in asteroids) stage.addChild(asteroid);
    ...
    public function update(deltaTime:Float, gameTime:Float):Void {
		...
		while (i >= 0) {
			...
			if (asteroid.isAlive) {
				asteroid.update(deltaTime, stage);
    ...
    public function exit(newState:GameState):Void {
		for (asteroid in asteroids) stage.removeChild(asteroid);
		asteroids = [];
		for (bullet in bullets) stage.removeChild(bullet);
		bullets = [];
